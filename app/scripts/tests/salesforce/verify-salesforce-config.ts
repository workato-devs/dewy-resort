...
import { getWorkatoSalesforceConfig, resetSalesforceClient } from '../lib/workato/config';
...
// Restore all environment variables before Test 5
if (originalTimeout) process.env.WORKATO_TIMEOUT = originalTimeout;
if (originalRetry) process.env.WORKATO_RETRY_ATTEMPTS = originalRetry;
else delete process.env.WORKATO_RETRY_ATTEMPTS;

// Test 5: Verify boolean parsing
console.log('Test 5: Testing boolean parsing...');
const booleanTests = [
  { env: 'WORKATO_MOCK_MODE', value: 'true', expected: true, field: 'mockMode' as const },
  { env: 'WORKATO_MOCK_MODE', value: 'false', expected: false, field: 'mockMode' as const },
  { env: 'WORKATO_CACHE_ENABLED', value: 'false', expected: false, field: 'cacheEnabled' as const },
  { env: 'WORKATO_CACHE_ENABLED', value: 'true', expected: true, field: 'cacheEnabled' as const },
];
...
// Restore environment
if (originalMock) process.env.WORKATO_MOCK_MODE = originalMock;
else delete process.env.WORKATO_MOCK_MODE;
if (originalCache) process.env.WORKATO_CACHE_ENABLED = originalCache;
else delete process.env.WORKATO_CACHE_ENABLED;
