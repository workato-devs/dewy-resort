...
/**
 * Test script for housekeeping MCP configuration
 * 
 * This script validates the housekeeping MCP configuration and tests connectivity
...
async function testHousekeepingMCPConfig() {
  logSection('Housekeeping MCP Configuration Test');
  ...
  try {
    ...
    if (config.role !== 'housekeeping') {
      log('✗ Invalid role: expected "housekeeping"', 'red');
      return false;
    }
    ...
    for (const server of config.servers) {
      ...
      // Validate required housekeeping tools
      const requiredTools = [
        'view_my_tasks',
    ...
    log('\nHousekeeping MCP configuration is ready!', 'green');
    return true;
