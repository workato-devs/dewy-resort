/**
 * Okta Error Scenarios Test Endpoint
 * Runs the test-okta-error-scenarios.js script and returns results
 */
...
export async function GET() {
  ...
  try {
    const { stdout, stderr } = await execAsync('node scripts/test-okta-error-scenarios.js', {
      timeout: 30000,
    });
    ...
    return NextResponse.json({
      success,
      message: success ? 'All automated error tests passed!' : 'Some error tests failed',
      summary: {
        total,
        passed,
        failed,
    ...
    return NextResponse.json({
      success: false,
      message: 'Error scenario tests failed to run',
      summary: {
        total: 0,
        passed: 0,
        failed: 1,
