...
if ($pythonCmd) {
    ...
    Write-Host "[OK] Removed workato-platform-cli from pip" -ForegroundColor Green
}

# Remove wrapper scripts
if (Test-Path "bin\workato.ps1") {
    Remove-Item "bin\workato.ps1" -Force
    Write-Host "[OK] Removed bin\workato.ps1" -ForegroundColor Green
}

if (Test-Path "bin\workato.cmd") {
    Remove-Item "bin\workato.cmd" -Force
    Write-Host "[OK] Removed bin\workato.cmd" -ForegroundColor Green
}
...
if (Test-Path "tools\workato-cli-env") {
    ...
    Write-Host "[OK] Removed legacy virtual environment" -ForegroundColor Green
}

Write-Host "[OK] Workato CLI cleanup completed" -ForegroundColor Green
