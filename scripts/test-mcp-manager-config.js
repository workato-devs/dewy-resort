...
/**
 * Test script for manager MCP configuration
 * 
 * This script validates the manager MCP configuration and tests connectivity
...
async function testManagerMCPConfig() {
  logSection('Manager MCP Configuration Test');
  
  try {
    // 1. Load configuration
    log('\n1. Loading manager MCP configuration...', 'blue');
    ...
    if (config.role !== 'manager') {
      log('✗ Invalid role: expected "manager"', 'red');
      return false;
    }
    log('✓ Role is correct: manager', 'green');
    ...
    const requiredTools = {
      'hotel-analytics': ['get_occupancy_stats', 'get_revenue_report'],
      'hotel-operations': ['view_all_bookings', 'assign_room']
    };
    ...
    for (const server of config.servers) {
      ...
      // Validate required tools for this server
      if (requiredTools[server.name]) {
        const missing = requiredTools[server.name].filter(tool => !server.tools.includes(tool));
        if (missing.length > 0) {
          log(`   ✗ Missing required tools: ${missing.join(', ')}`, 'red');
          return false;
        }
        log(`   ✓ All required tools are configured`, 'green');
      }
    }
    ...
    log('\nConfigured manager tools:', 'cyan');
    for (const server of config.servers) {
      log(`  ${server.name}:`, 'blue');
    ...
    log('\nManager MCP configuration is ready!', 'green');
    return true;
