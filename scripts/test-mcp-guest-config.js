...
/**
 * Test script for guest MCP configuration
 * 
 * This script validates the guest MCP configuration and tests connectivity
...
async function testGuestMCPConfig() {
  logSection('Guest MCP Configuration Test');
  
  try {
    // 1. Load configuration
    log('\n1. Loading guest MCP configuration...', 'blue');
    const configPath = path.join(process.cwd(), 'config', 'mcp', 'guest.json');
    ...
    if (config.role !== 'guest') {
      log('✗ Invalid role: expected "guest"', 'red');
      return false;
    }
    log('✓ Role is correct: guest', 'green');
    ...
    log('\nGuest MCP configuration is ready!', 'green');
    return true;
