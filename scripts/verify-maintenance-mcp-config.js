...
/**
 * Verify maintenance MCP configuration can be loaded by MCP Manager
 */
...
async function verifyMaintenanceConfig() {
  logSection('Maintenance MCP Configuration Verification');
  ...
  try {
    ...
    if (config.role !== 'maintenance') {
      log(`✗ Invalid role: ${config.role} (expected "maintenance")`, 'red');
      return false;
    }
    ...
    for (const server of config.servers) {
      ...
      // Check for required maintenance tools
    ...
    if (allRequiredToolsPresent) {
      log('\n✓ All required maintenance tools are configured', 'green');
    } else {
    ...
    log('\nMaintenance MCP configuration is valid and ready to use!', 'green');
    return true;
